...
const CoordinatorSubmissions = () => {
  ...
  /**
   * Retorna a URL do primeiro certificado da submissão.
   * Suporta tanto URLs normais quanto base64.
   */
  const getCertificadoUrl = (sub: SubmissaoResponse): string => {
  ...
  /**
   * Detecta o tipo do arquivo a partir da URL ou base64
   */
  const isPdf = (url: string): boolean => {
    if (isBase64(url)) return url.startsWith("data:application/pdf");
    return url.toLowerCase().endsWith(".pdf");
  };

  /**
   * Faz o download do arquivo — funciona para base64 e URLs normais
   */
  const handleDownload = (sub: SubmissaoResponse) => {
    const url = getCertificadoUrl(sub);
    if (!url) return;

    const nomeArquivo = sub.certificados?.[0]?.nomeArquivo ?? "certificado";

    if (isBase64(url)) {
      // Cria um link temporário para download do base64
      ...
      // URL normal — abre em nova aba
      window.open(url, "_blank");
    }
  };

  /**
   * Renderiza o preview do certificado no modal
   */
    ...
    <div className="flex flex-col p-8 space-y-6 bg-slate-50 min-h-screen">
              ...
              <div className="flex flex-col space-y-4 bg-slate-50 p-6 border-r border-slate-100 overflow-hidden">
                <div className="flex justify-between items-center">
                  ...
                  <Button
                    ...
                    disabled={!getCertificadoUrl(selected)}
                  >
                ...
                <div className="flex-1 border-2 border-slate-200 border-dashed rounded-2xl overflow-hidden bg-white shadow-inner">
                  {renderPreview(getCertificadoUrl(selected))}
                </div>
              </div>
              ...
              <div className="flex flex-col space-y-6 p-8 overflow-y-auto bg-white">
                <div className="grid grid-cols-2 gap-6 p-5 rounded-2xl bg-slate-50/50 border border-slate-100 shadow-sm">
                  ...
                  <div>
                    ...
                    <div className="mt-1">
                      <Badge className={statusBadges[selected.status]}>{selected.status}</Badge>
                    </div>
                  </div>
                  <div>
      ...
      <Dialog open={!!confirmRejectSub} onOpenChange={(open) => !open && setConfirmRejectSub(null)}>
        <DialogContent className="sm:max-w-[400px] rounded-2xl">
          <DialogHeader>
            <DialogTitle className="text-red-600 font-bold">Confirmar Reprovação</DialogTitle>
          </DialogHeader>
          ...
          <DialogFooter className="mt-4 gap-2">
            ...
            <Button
              ...
              onClick={() => confirmRejectSub && handleAction(confirmRejectSub.id, "REPROVADA")}
            >
              Sim, Reprovar
            </Button>
          </DialogFooter>
